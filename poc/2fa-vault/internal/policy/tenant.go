...
func getVaultTx(tx *sql.Tx, vaultID string) (*VaultRecord, error) {
	var v VaultRecord
	err := tx.QueryRow(`
SELECT vault_id, template_version, policy_version, network, rp_id, origin,
       phone_routine_bip340_compressed, phone_direct_p256_compressed,
...
func insertVaultCredentialTx(tx *sql.Tx, cred VaultCredential) error {
	_, err := tx.Exec(`
INSERT INTO vault_credential (credential_id, vault_id, webauthn_p256_compressed, user_handle, resident, integrity_mac)
VALUES (?,?,?,?,?,?)`,
		cred.CredentialID, cred.VaultID, cred.WebAuthnP256, cred.UserHandle, boolToInt(cred.Resident), cred.IntegrityMAC,
...
func credentialIdentitiesEqual(got, want *Credential) error {
	if got == nil && want == nil {
		return nil
	}
	if got == nil || want == nil {
		return fmt.Errorf("credential presence")
	}
	switch {
	case !bytes.Equal(got.ID, want.ID):
		return fmt.Errorf("credential_id")
	case got.VaultID != want.VaultID:
		return fmt.Errorf("vault_id")
	case got.RPID != want.RPID:
	...
	case got.FeerateCapSatPerV != want.FeerateCapSatPerV:
		return fmt.Errorf("feerate_cap_sat_vb")
	case !bytes.Equal(got.IntegrityMAC, want.IntegrityMAC):
		return fmt.Errorf("integrity_mac")
	default:
		return nil
	}
}
