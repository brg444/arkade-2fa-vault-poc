...
func TestTwoTenantsHaveIsolatedDescriptorsCapsAndLoaders(t *testing.T) {
	...
	if err := svc.CreateTenantVault(tenantB, token, RegisterRequest{
	...
	if _, _, err := led.Issue(context.Background(), tenantB, digest, 1_000, 100, fixture.PeriodAllowanceSats, func(context.Context) (string, error) {
		return "signed-b", nil
	}); err != nil {
		t.Fatal(err)
	}
