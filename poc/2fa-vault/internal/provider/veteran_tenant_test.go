...
func TestVeteranTwoTenantsTwoScriptsAndIsolatedIssuance(t *testing.T) {
	...
	if _, _, err := led.Issue(context.Background(), fixture.VaultID, digest, 1_000, 100, fixture.PeriodAllowanceSats, func(context.Context) (string, error) {
		return "signed-a", nil
	}); err != nil {
		t.Fatal(err)
	}
	if _, _, err := led.Issue(context.Background(), tenantB, digest, 2_000, 100, fixture.PeriodAllowanceSats, func(context.Context) (string, error) {
		return "signed-b", nil
	}); err != nil {
		t.Fatal(err)
	}
	...
	signed, replay, err := led.Issue(context.Background(), fixture.VaultID, digest, 1_000, 100, fixture.PeriodAllowanceSats, func(context.Context) (string, error) {
...
func TestVeteranMidnightDoesNotRefillEitherTenant(t *testing.T) {
	...
	if _, _, err := led.Issue(context.Background(), fixture.VaultID, bytes.Repeat([]byte{0x31}, 32), 90, 3, 100, func(context.Context) (string, error) {
		return "a", nil
	}); err != nil {
		t.Fatal(err)
	}
	if _, _, err := led.Issue(context.Background(), tenantB, bytes.Repeat([]byte{0x32}, 32), 90, 3, 100, func(context.Context) (string, error) {
		return "b", nil
	}); err != nil {
		t.Fatal(err)
	}
	clock.now = time.Date(2026, 8, 16, 0, 0, 0, 0, time.UTC)
	if _, _, err := led.Issue(context.Background(), fixture.VaultID, bytes.Repeat([]byte{0x33}, 32), 90, 3, 100, func(context.Context) (string, error) {
		return "refill-a", nil
	}); err == nil {
		t.Fatal("midnight refilled A")
	}
	if _, _, err := led.Issue(context.Background(), tenantB, bytes.Repeat([]byte{0x34}, 32), 90, 3, 100, func(context.Context) (string, error) {
		return "refill-b", nil
	}); err == nil {
