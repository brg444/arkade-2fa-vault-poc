...
type PublicTweaks struct {
	Routine  PublicPair                       `json:"routine"`
	Initiate map[string]map[string]PublicPair `json:"initiate"`
...
func BuildPublicDescriptor(in FamilyInput, origin, version string) (PublicDescriptor, *Family, error) {
	...
	d := PublicDescriptor{
		...
		Tweaks: PublicTweaks{
			Routine: PublicPair{
				Vault:  hex.EncodeToString(in.RoutineVault.SerializeCompressed()),
				Arkade: hex.EncodeToString(in.RoutineArkade.SerializeCompressed()),
			},
			Initiate: map[string]map[string]PublicPair{
				"daily":   pairMap(fam.InitiateDaily),
				"savings": pairMap(fam.InitiateSave),
			},
...
func encodePublicDescriptor(d PublicDescriptor) ([]byte, error) {
	...
	for _, field := range []struct {
		...
		{d.Tweaks.Routine.Arkade, "routine.arkade"},
	} {
		if err := appendExactHex(&parts, field.hex, field.name, 33); err != nil {
			return nil, err
		}
	...
	for _, kind := range []string{"daily", "savings"} {
		for _, claimant := range familyClaimants(hasRecovery) {
			...
			if err := appendExactHex(&parts, pair.Arkade, "initiate."+kind+"."+claimant+".arkade", 33); err != nil {
				return nil, err
			}
		}
	}
	...
	if err := appendExactHex(&parts, d.Daily.Script, "daily.script", len(d.Daily.Script)/2); err != nil {
		return nil, err
	}
	if err := appendCanonText(&parts, d.Daily.Address, "daily.address"); err != nil {
		return nil, err
	}
